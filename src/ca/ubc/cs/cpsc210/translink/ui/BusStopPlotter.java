...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
       // System.out.println("markStops called");
        newStopClusterer();
        ...
        {
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())) //&& !(stopMarkerMap.containsKey(s)))
            {
                ...
                if (nearestStnMarker == null || (!(nearestStnMarker.getRelatedObject().equals(s))))
                {
                    m.setIcon(stopIconDrawable);
                }

                stopMarkerMap.put(s, m);
                stopClusterer.add(m);
